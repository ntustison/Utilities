...
static const char *optString = "a:f:m:j:k:w:c:d:o:O:r:n:b:i:s:g:x:p:y:z:l:R:B:G:J:X:N:I:C:L:D:S:P:F:A:at:ev::h?";
...
struct arguments
  {
   ...
   std::string inputControlPointLatticeFile;    /* -A option */
...
void display_usage( const std::string progname )
{
   ...
   std::cout << "  -n/--num-levels = UINT                        Number of multiresolution levels, default: " << defargs.numLevels << std::endl;
...
//    registrationFilter->SetInput( 1, movingrescalefilter->GetOutput() );
  ...
  typedef itk::PDEDeformableRegistrationFunction<RealImageType,
...
//  registrationFilter->DebugOn();
  ...
  for ( unsigned int i = 0; i < ImageDimension; i++ )
    ...
    RealType fixedFactor = vnl_math_min( pow( 2, args.numLevels-1 ), fixedSize/32.0 );
    RealType movingFactor = vnl_math_min( pow( 2, args.numLevels-1 ), movingSize/32.0 );
