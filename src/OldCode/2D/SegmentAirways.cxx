...
int main( int argc, char *argv[] )
{
  ...
  for ( int n = seedSliceIndex; n >= 0; n-- )
    ...
    stats->SetInput( labelextracter->GetOutput() );
    stats->Update();

    if ( stats->GetMaximum() >= 2 )
      {
  ...
  for ( iter = circles.begin(); iter != circles.end(); ++iter )
    ...
    LabelSliceType::PixelType centerValue = labelExtracter->GetOutput()->GetPixel( index );
