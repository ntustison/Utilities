library(ANTsR)
asl_img<-'asl.nii.gz'  # 4D nii image
moco_results <- motion_correction( asl_img )
# these params need tuning for your data --- crude brain mask
moco_mask_img <- get_mask( moco_results$moco_avg_img , thresh_lo = 500
, thresh_hi = 1e9 )
...
cbf <- perfusionregression( moco_mask_img, mat , predictors$xideal ,
predictors$nuis )
...
# cbf <- perfusionregression( moco_mask_img, mat , predictors$xideal ,
predictors$nuis , TRUE ) # takes much longer
...
M0 <- 0.025 # rowMeans( controlimg ) --- can be a better way for PASL
lambda <- 0.9
alpha <- 0.68
T1b <- 1664
omega <- 1
