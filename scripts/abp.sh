...
EXTRACTION_MATRIX_OFFSET=${EXTRACTION_WARP_OUTPUT_PREFIX}0MatrixOffset.mat
...
if [[ ! -f ${EXTRACTION_MASK} || ! -f ${EXTRACTION_WM} ]];
  then
    ...
    TMP_FILES=( $EXTRACTION_MASK_PRIOR_WARPED $EXTRACTION_WARP $EXTRACTION_INVERSE_WARP $EXTRACTION_MATRIX_OFFSET $EXTRACTION_TMP $EXTRACTION_MASK_TMP $EXTRACTION_GM $EXTRACTION_CSF $EXTRACTION_SEGMENTATION $EXTRACTION_SKULL_TOP $EXTRACTION_TEMPLATE_SKULL_TOP )
