...
ANTS_TRANSFORMATION="BSplineSyN[0.1,10x10x11,0x0x0]"
...
if [[ ! -f ${EXTRACTION_MASK} || ! -f ${EXTRACTION_WM} ]];
  then
    ...
    if [[ ! -f ${EXTRACTION_WARP} ]];
      ...
      stage3=" -m CC[${EXTRACTION_TEMPLATE},${N4_CORRECTED_IMAGES[0]},0.5,4] -m CC[${EXTRACTION_TEMPLATE_LAPLACIAN},${EXTRACTION_LAPLACIAN},0.5,4] -c [50x10x0,1e-9,15] -t BSplineSyN[0.1,10x10x11,0x0x0] -f 4x2x1 -s 2x1x0";
...
if [[ ! -f ${BRAIN_SEGMENTATION} ]];
  then
    ...
    if [[ ! -f ${SEGMENTATION_WARP} ]];
      ...
      basecall="${ANTS} -d ${DIMENSION} -u 1 -w [0.025,0.975] -o ${SEGMENTATION_WARP_OUTPUT_PREFIX} -r [${N4_CORRECTED_IMAGES[0]},${SEGMENTATION_TEMPLATE_MASKED},1] -z 1"
