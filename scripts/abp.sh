...
Required arguments:
     ...
     -o:  OutputPrefix                          The following images are created using the specified prefix:
                                                  ...
                                                  * ${OUTPUT_PREFIX}SurfaceCurvature.${OUTPUT_SUFFIX}
...
if [[ ! -f ${EXTRACTION_MASK} || ! -f ${EXTRACTION_WM} ]];
  then
    ...
    TMP_FILES=( $EXTRACTION_MASK_PRIOR_WARPED $EXTRACTION_WARP $EXTRACTION_INVERSE_WARP $EXTRACTION_MATRIX_OFFSET $EXTRACTION_TMP $EXTRACTION_MASK_TMP $EXTRACTION_GM $EXTRACTION_CSF $EXTRACTION_SEGMENTATION $EXTRACTION_INITIAL_AFFINE $EXTRACTION_INITIAL_AFFINE_MOVING $EXTRACTION_INITIAL_AFFINE_FIXED )

    ## Step 1 ##
    if [[ ! -f ${EXTRACTION_WARP} ]];
      then

#     TMP_FILES=( $EXTRACTION_MASK_PRIOR_WARPED $EXTRACTION_WARP $EXTRACTION_INVERSE_WARP $EXTRACTION_MATRIX_OFFSET $EXTRACTION_TMP $EXTRACTION_MASK_TMP $EXTRACTION_GM $EXTRACTION_CSF $EXTRACTION_SEGMENTATION $EXTRACTION_SKULL_TOP $EXTRACTION_TEMPLATE_SKULL_TOP $EXTRACTION_INITIAL_AFFINE $EXTRACTION_INITIAL_AFFINE_MOVING $EXTRACTION_INITIAL_AFFINE_FIXED )
...
#       basecall="${ANTS} -d ${DIMENSION} -u 1 -w [0.025,0.975] -o ${EXTRACTION_WARP_OUTPUT_PREFIX} -r [${EXTRACTION_TEMPLATE_SKULL_TOP},${EXTRACTION_SKULL_TOP},1] -z 1"
      ...
      exe_initial_align="${ANTSPATH}/antsAffineInitializer ${DIMENSION} ${EXTRACTION_INITIAL_AFFINE_FIXED} ${EXTRACTION_INITIAL_AFFINE_MOVING} ${EXTRACTION_INITIAL_AFFINE} 15 0.3"
      ...
      stage3="-m CC[${EXTRACTION_TEMPLATE},${N4_CORRECTED_IMAGES[0]},1,4] -c [50x20x0,1e-9,15] -t BSplineSyN[0.25,10x10x11,0x0x0] -f 4x2x1 -s 2x1x0";
...
if [[ -f ${REGISTRATION_TEMPLATE} ]];
  ...
  REGISTRATION_TEMPLATE_MATRIX_OFFSET=${REGISTRATION_TEMPLATE_OUTPUT_PREFIX}0MatrixOffset.mat
