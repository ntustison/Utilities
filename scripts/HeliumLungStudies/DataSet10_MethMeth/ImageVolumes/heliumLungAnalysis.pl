...
@args = ( $utilityDirectory . "OtsuThresholdImage", 3,
  $initialSegmentation, $initialSegmentation, 1 );
system( @args ) == 0 or die "system @args failed: $?";
@args = ( $utilityDirectory . "ThresholdImage", 3,
  $initialSegmentation, $initialSegmentation, 2, 2, 1, 0 );
system( @args ) == 0 or die "system @args failed: $?";
@args = ( $utilityDirectory . "BinaryMorphology", 3,
  $initialSegmentation, $initialLevelSet, 1, 2, 1, 1 );
system( @args ) == 0 or die "system @args failed: $?";
...
@args = ( $utilityDirectory . "InhomogeneityCorrectImage", 3,
  $changeInformation, $heliumImage, 1, $initialSegmentation, 50, 4,
  $outputDirectory . "/bias_field_" . $PID . ".nii.gz" );
system( @args ) == 0 or die "system @args failed: $?";
...
$apocritaImage = $outputDirectory . "apocrita_both_${PID}.nii.gz";
...
$output = $outputDirectory . "/apocrita_ventilation_ratio_both_" . $PID . ".txt";
$out = `$command 3 $apocritaImage 2 $initialSegmentation 1 > $output`;

$tmpApocritaImage = $outputDirectory . "apocrita_2_${PID}.nii.gz";
$command = $utilityDirectory . "ExtractMaskImage";
$out = `$command 3 $apocritaImage $mask[0] $tmpApocritaImage 2 0`;
$command = $utilityDirectory . "CalculateVolumeFromBinaryImage";
$output = $outputDirectory . "/apocrita_ventilation_ratio_2_" . $PID . ".txt";
$out = `$command 3 $tmpApocritaImage 2 $mask[0] 2 > $output`;
unlink( $tmpApocritaImage );

$tmpApocritaImage = $outputDirectory . "apocrita_3_${PID}.nii.gz";
$command = $utilityDirectory . "ExtractMaskImage";
$out = `$command 3 $apocritaImage $mask[0] $tmpApocritaImage 3 0`;
$command = $utilityDirectory . "CalculateVolumeFromBinaryImage";
$output = $outputDirectory . "/apocrita_ventilation_ratio_3_" . $PID . ".txt";
$out = `$command 3 $tmpApocritaImage 2 $mask[0] 3 > $output`;
unlink( $tmpApocritaImage );
