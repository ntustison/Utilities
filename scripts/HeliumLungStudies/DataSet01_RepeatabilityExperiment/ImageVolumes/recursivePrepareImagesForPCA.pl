  ...
  if( $filename =~ m/^Series/ && $filename =~ m/.nii.gz$/  )
    ...
    if( $count < 10 )
      ...
      @args = ( "cp",  $directories . $filename, 
        "/home/tustison/Data/HeliumLungStudies/PrePostAlbuterol/ImageVolumes/PCA/Repeatability/he3_0${count}.nii.gz" );
      system( @args ) == 0 or die "system @args failed: $?";
      }
    else
      {
      @args = ( "cp",  $directories . $filename, 
        "/home/tustison/Data/HeliumLungStudies/PrePostAlbuterol/ImageVolumes/PCA/Repeatability/he3_${count}.nii.gz" );
      system( @args ) == 0 or die "system @args failed: $?";
      }
    $count++;  
