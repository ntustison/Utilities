...
fix_headers=$3  # set fix_headers=0 unless there is a gross misregistration in ITK-Snap
...
if [ $run_register -eq 1 ]; then
  ...
  if [ $ANTSINTRO -eq 1 ]; then
    ...
    exe="${ANTSPATH}ANTS $DIM -m PR[$OUT_HEAD_N3,$TEMPLATE,1,4] -o ${OUT}${FORMAT} $PARAMS"
    echo $exe; $exe
  fi
fi
...
if [ $run_atropos -eq 1 ]; then
  PARAMS1="-n [4,0.0001] -x $MASKB -m [$MRF,1] -u false -h 0 -e 0"
...
if [ $run_postprocess2 -eq 1 ]; then
  ...
  if [ $skip_postprocess1 -eq 1 ]; then
    ${ANTSPATH}MultiplyImages $DIM $MASK_TEMPLATE $INPUT_FILE $OUT_MASKED
    ${ANTSPATH}Atropos $DIM -a $OUT_MASKED -x $MASK_TEMPLATE -m [0.25,1] -i Kmeans[3] -o $SEG2 -c [0.001,2]
  else
    ${ANTSPATH}Atropos $DIM -a $OUT_BRAIN_N3 -x $OUT_MASK -m [0.25,1] -i Kmeans[3] -o $SEG2 -c [0.001,2]
  fi
  ...
  # Morphological close with fill (to fill in tunnels, such as removed veins)
  ${ANTSPATH}ImageMath $DIM $OUT_MASK2 MD $OUT_MASK2 3
  ${ANTSPATH}ImageMath $DIM $OUT_MASK2 FillHoles $OUT_MASK2
  ${ANTSPATH}ImageMath $DIM $OUT_MASK2 ME $OUT_MASK2 3
  # Apply filled mask
