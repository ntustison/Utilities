...
public:
  ...
  /** Get the center of mass of a particular object in physical space (in units of pixel
   * size). This information is only valid after the filter has
   * executed. Size of the background (object #0) is not calculated.  */
  PointType GetCenterOfMass(unsigned long obj) const
    {
      if (obj > 0 && obj <= m_NumberOfObjects)
        {
        return m_CenterOfMassOfObjects[obj-1];
        }
...
private:
  ...
  std::vector<PointType> m_CenterOfMassOfObjects;
