#ifndef _itkBinaryDilateImageFilter_hxx
#define _itkBinaryDilateImageFilter_hxx
...
{
  this->m_BoundaryIsForeground = false;
}
...
::GenerateData()
{
  ...
  // compute the size of the temp image. It is needed to create the progress reporter
  ...
  if( this->m_BoundaryIsForeground )
    { tmpImage->FillBuffer( onTag ); }
  else
    { tmpImage->FillBuffer( backgroundTag ); }

  // Iterators on input and tmp image
  ImageRegionConstIterator<TInputImage> iRegIt( input, requiredInputRegion );   // iterator on input
  ImageRegionIterator<TempImageType> tmpRegIt( tmpImage, requiredInputRegion );    // iterator on tmp image
  ...
  typename Superclass::ComponentVectorConstIterator vecBeginIt = this->KernelCCVectorBegin();
  typename Superclass::ComponentVectorConstIterator vecEndIt = this->KernelCCVectorEnd();
