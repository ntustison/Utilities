...
::GenerateData()
{
  ...
  RelabelComponentObjectType initialAttributes;
  initialAttributes.m_SizeInPixels = 1;
  initialAttributes.m_SizeInPhysicalUnits = physicalPixelSize;
  ( initialAttributes.m_CenterOfMass ).Fill( 0.0 );
  ...
  it = ImageRegionConstIteratorWithIndex<InputImageType>(input,
  ...
  while (!it.IsAtEnd())
    ...
    if (inputValue != NumericTraits<InputPixelType>::Zero)
      ...
      if ( mapIt == sizeMap.end() )
        ...
        initialAttributes.m_ObjectNumber = inputValue;
        PointType point;
        this->GetInput()->TransformIndexToPhysicalPoint( it.GetIndex(), point );
        for ( unsigned int d = 0; d < ImageDimension; d++ )
          {
          initialAttributes.m_CenterOfMass[d] = point[d];
          }
        sizeMap.insert( MapValueType( inputValue, initialAttributes ) );
        }
      else
        {
        ...
        for ( unsigned int d = 0; d < ImageDimension; d++ )
          {
          (*mapIt).second.m_CenterOfMass[d] += point[d];
          }
        }
      }
  ...
  m_CenterOfMassOfObjects.resize(m_NumberOfObjects);
  ...
  for (i=0, vit = sizeVector.begin(); vit != sizeVector.end(); ++vit, ++i)
    ...
    if (m_MinimumObjectSize > 0 && (*vit).m_SizeInPixels < m_MinimumObjectSize)
      ...
      for ( unsigned int d = 0; d < ImageDimension; d++ )
        {
        centerOfMass[d] = (*vit).m_CenterOfMass[d] /
          static_cast<double>( (*vit).m_SizeInPixels );
        }
      m_CenterOfMassOfObjects[i] = centerOfMass;
      }
  ...
  ImageRegionConstIterator<InputImageType> iit;
  iit = ImageRegionConstIterator<InputImageType>(input,
                                                 output->GetRequestedRegion());
