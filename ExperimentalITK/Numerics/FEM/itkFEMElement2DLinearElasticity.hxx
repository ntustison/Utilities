/*=========================================================================
  ...
  Module:    $RCSfile: itkFEMElement2DLinearElasticity.hxx,v $
  Language:  C++
...
#ifndef __itkFEMElement2DLinearElasticity_hxx
#define __itkFEMElement2DLinearElasticity_hxx
...
Element2DLinearElasticity<TBaseClass>
::Element2DLinearElasticity() : Superclass(), m_mat(0) {}
...
::GetMaterialMatrix(MatrixType& D) const
{
  ...
  Float lambda = m_mat->nu*m_mat->E 
                /((1.0+m_mat->nu)*(1.0-2.0*m_mat->nu));
  Float mu =  0.5*m_mat->E/(1.0+m_mat->nu);
...
::Read( std::istream& f, void* info )
{
  ...
  {
    throw FEMExceptionObjectNotFound(__FILE__,__LINE__,"Element2DLinearElasticity::Read()",e.m_baseClassName,e.m_GN);
  }

  // Check if the material object was of correct class
  if(!m_mat)
  {
    throw FEMExceptionWrongClass(__FILE__,__LINE__,"Element2DLinearElasticity::Read()");
  }
...
out:
  ...
  if( !f )
  { 
    throw FEMExceptionIO(__FILE__,__LINE__,"Element2DLinearElasticity::Read()","Error reading FEM element!");
  }
...
::Write( std::ostream& f ) const
{
  ...
  if (!f)
  { 
    throw FEMExceptionIO(__FILE__,__LINE__,"Element2DLinearElasticity::Write()","Error writing FEM element!");
  }
}
...
#endif // #ifndef __itkFEMElement2DLinearElasticity_hxx
