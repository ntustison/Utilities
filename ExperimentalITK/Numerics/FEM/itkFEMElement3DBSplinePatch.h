/*=========================================================================
  ...
  Module:    $RCSfile: itkFEMElement3DBSplinePatch.h,v $
  Language:  C++
...
#ifndef __itkFEMElement3DBSplinePatch_h
#define __itkFEMElement3DBSplinePatch_h
...
/**
 * \class Element3DBSplinePatch
 * \brief C^(BSplineOrder-2) continuous finite element in 3D space.
 */
class Element3DBSplinePatch : public Element
{
typedef Element TemplatedParentClass;
FEM_ABSTRACT_CLASS( Element3DBSplinePatch, TemplatedParentClass )

  /*************************************************************************/
  //  The first part of this class contains information related to FEM-
  //  based B-spline image registration when using the entire 3-D image.
  //  Most of the functions are adapted to also employ *extended* B-splines.
  /*************************************************************************/
public:

  Element3DBSplinePatch() : m_BSplineOrder(3), m_NumberOfNodes(27) 
   { 
     this->GenerateShapeFunctions(); 
     m_localCoordinateBoundaries.set_size(6);  
     ...
     m_localCoordinateBoundaries[4] = 0.0;
     m_localCoordinateBoundaries[5] = 1.0;
   ...
   */
  enum { NumberOfSpatialDimensions = 3 };
  ...
  virtual unsigned int GetNumberOfCorners( void ) const { return 8; }  
   ...
   */
  /** Set/Get BSplineOrder (Default order = 3) */ 
  void SetBSplineOrder(unsigned int order) 
    { 
      m_BSplineOrder = (order >= 2) ? order : 2; 
      this->SetNumberOfNodes(m_BSplineOrder*m_BSplineOrder*m_BSplineOrder);
      this->GenerateShapeFunctions();
    };
...
private:  
   ...
   *                               contains those limits (length = 6 --- [u0, u1, v0, v1, w0, w1]).
   */
...
#endif  // #ifndef __itkFEMElement3DBSplinePatch_h
