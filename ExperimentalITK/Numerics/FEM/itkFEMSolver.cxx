 ...
 */
Solver::Solver() : NGFN(0), NMFC(0), m_NZE(0)
{
...
void Solver::InitializeInterpolationGrid(const VectorType& size, const VectorType& bb1, const VectorType& bb2)
{
  ...
  for(ElementArray::iterator e=el.begin(); e!=el.end(); e++)
  {
    ...
    v1=(*e)->GetCornerCoordinates(0); // lower left corner
    v2=v1; // upper right corner

    const unsigned int NumberOfDimensions=(*e)->GetNumberOfSpatialDimensions();


    for(unsigned int n=1; n < (*e)->GetNumberOfCorners(); n++ )
    {
      const VectorType& v=(*e)->GetCornerCoordinates(n);
...
unsigned long 
Solver::GetMaximumNumberOfNonZeroElements( void ) 
{
  unsigned int N = 0;
  for(ElementArray::iterator e = el.begin(); e != el.end(); e++) 
  {      
    unsigned long Npts=(*e)->GetNumberOfNodes();    
    m_NZE += (Npts*(Npts-1) >> 1);
  }
...
Solver::GetElementAtPoint(const VectorType& pt) const
{
  ...
  {
    //if (m_InterpolationGrid->GetPixel(index) == NULL)
    //   std::cout << "if: index: " << index << ", pp = " << pp << "\n";
