...
void Element::GetStiffnessMatrix(MatrixType& Ke) const
{
  ...
  this->JacobianInverse(ip,invJ,&J);  
  shapeDgl=invJ*shapeD;
...
Element::GetGlobalFromLocalCoordinates( const VectorType& pt ) const
{
  ...
  {
    VectorType Gl(this->GetNumberOfSpatialDimensions());
    Gl = 0.0;
    return Gl;
  }
}
...
Element::VectorType
Element::GetSolutionGlobalFromLocalCoordinates(const VectorType& pt, Solution::ConstPointer sol) const
{
  unsigned int Nnodes = this->GetNumberOfNodes();
  if (Nnodes > 0)
  {
    MatrixType nc(this->GetNumberOfSpatialDimensions(), Nnodes);
    VectorType solPt(this->GetNumberOfSpatialDimensions());
    for(unsigned int n = 0; n < Nnodes; n++)
    {
      for (unsigned int i = 0; i < this->GetNumberOfSpatialDimensions(); i++)
        solPt[i] = sol->GetSolutionValue(this->GetNode(n)->GetDegreeOfFreedom(i));
      nc.set_column(n, this->GetNodeCoordinates(n) + solPt);      
    }
    VectorType shapeF = ShapeFunctions(pt);
    return nc*shapeF;
  }
