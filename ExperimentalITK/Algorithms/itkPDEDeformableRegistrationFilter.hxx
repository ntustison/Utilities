/*=========================================================================
  ...
  Module:    $RCSfile: itkPDEDeformableRegistrationFilter.hxx,v $
  Language:  C++
...
=========================================================================*/
#ifndef _itkPDEDeformableRegistrationFilter_hxx_
#define _itkPDEDeformableRegistrationFilter_hxx_
...
void
PDEDeformableRegistrationFilter<TFixedImage,TMovingImage,TDeformationField>
::SmoothDeformationField(typename TDeformationField::Pointer field, float sig)
{

  if (!field) field = this->GetOutput();
  ...
  for( unsigned int j = 0; j < ImageDimension; j++ )
    ...
    oper->SetDirection( j );
    double variance;
    if (sig > 0) variance = vnl_math_sqr( sig );
    else variance = fabs( m_StandardDeviations[j] );
