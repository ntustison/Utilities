...
Module:    $RCSfile: itkDMFFDLabeledPointSetRegistrationFilter.hxx,v $
Language:  C++
...
#ifndef __itkDMFFDLabeledPointSetRegistrationFilter_hxx
#define __itkDMFFDLabeledPointSetRegistrationFilter_hxx
...
::DMFFDLabeledPointSetRegistrationFilter()
{
  ...
  this->m_AnnealingRate = 0.97;
  this->m_PointSetSigmas.SetSize( this->m_MaximumNumberOfIterations.Size() );
  this->m_PointSetSigmas.Fill( 1.0 );
  this->m_Alpha = 1.0;
...
::Initialize()
{
  if( this->m_CurrentLevel == 0 )
    ...
    this->m_CurrentPointSetSigma = this->m_PointSetSigmas[0];
    ...
    {
    if( this->m_PointSetSigmas.Size() > this->m_CurrentLevel )
      {
      this->m_CurrentPointSetSigma = this->m_PointSetSigmas[this->m_CurrentLevel];
      this->m_CurrentAnnealing = 1.0;
      }
...
::CalculateInitialCentroidsAndNorms()
{
  ...
  for( unsigned int i = 0; i < this->m_PointSetSigmas.size(); i++ )
    {
    this->m_PointSetSigmas[i] *= vcl_sqrt( fixedPointSetNorm );
    }
    ...
    if( this->m_Verbose )
      ...
      std::cout << "  Iteration " << its
        ...
        << vcl_sqrt( this->m_CurrentAnnealing / this->m_AnnealingRate )
           * this->m_CurrentPointSetSigma << std::endl;
      }
    itkDebugMacro( "  Iteration " << its
      << " (of " << this->m_MaximumNumberOfIterations[this->m_CurrentLevel]
      << "): Energy = " << fp << ", " << " Annealing temperature = "
      << vcl_sqrt( this->m_CurrentAnnealing / this->m_AnnealingRate )
         * this->m_CurrentPointSetSigma );
...
template<class TFixedPointSet, class TMovingPointSet, class TWarpedPointSet>
typename DMFFDLabeledPointSetRegistrationFilter
...
::EvaluateMetric( RealType t = 0 )
{
  ...
  pointSetFunction->SetUseRegularizationTerm( this->m_UseRegularizationTerm );
  pointSetFunction->SetAlpha( this->m_Alpha );

  pointSetFunction->SetFixedPointSet( this->GetInput( 0 ) );
  pointSetFunction->SetFixedPointSetSigma( this->m_CurrentPointSetSigma
  ...
  pointSetFunction->SetMovingPointSetSigma( this->m_CurrentPointSetSigma
    * vcl_sqrt( this->m_CurrentAnnealing ) );
...
::PrintSelf(std::ostream& os, Indent indent) const
{
  ...
  os << indent << "Point-set sigmas: "
     << this->m_PointSetSigmas << std::endl;
