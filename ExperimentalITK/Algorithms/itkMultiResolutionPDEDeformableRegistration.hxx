/*=========================================================================
  ...
  Module:    $RCSfile: itkMultiResolutionPDEDeformableRegistration.hxx,v $
  Language:  C++
...
=========================================================================*/
#ifndef _itkMultiResolutionPDEDeformableRegistration_hxx
#define _itkMultiResolutionPDEDeformableRegistration_hxx
...
::GenerateData()
{
  ...
  DeformationFieldPointer inverseField=NULL;
...
std::cout << " expand factors " << expandFactors[idim] << std::endl;
      ...
      if (m_InverseField) 
      {
        std::cout << " SETTING NEW ENLARGED INVERSE FIELD " << std::endl;
        m_RegistrationFilter->SetInverseField( this->ExpandField(m_InverseField , expandFactors));
      }
    }
...
//    m_RegistrationFilter->SetMovingImage( m_FixedImagePyramid->GetOutput(movingLevel) );
//    m_RegistrationFilter->SetFixedImage( m_MovingImagePyramid->GetOutput(fixedLevel) );
//    m_RegistrationFilter->SetOriginalFixedImage( m_MovingImagePyramid->GetOutput(fixedLevel) );
//    m_RegistrationFilter->SetOriginalMovingImage( m_FixedImagePyramid->GetOutput(fixedLevel) );
    m_RegistrationFilter->SetMovingImage(  m_MovingImagePyramid->GetOutput(fixedLevel) );
    m_RegistrationFilter->SetFixedImage(m_FixedImagePyramid->GetOutput(movingLevel) );
    m_RegistrationFilter->SetOriginalFixedImage( m_FixedImagePyramid->GetOutput(fixedLevel) );
    m_RegistrationFilter->SetOriginalMovingImage(  m_MovingImagePyramid->GetOutput(fixedLevel) );
...
//        this->GraftOutput( m_RegistrationFilter->GetOutput() );
       m_InverseField = m_RegistrationFilter->GetInverseField();
	if (!m_InverseField) std::cout << " PDEMULTRES NO INV FI " << std::endl;
...
MultiResolutionPDEDeformableRegistration<TFixedImage,TMovingImage,TDeformationField>
::ExpandField(DeformationFieldPointer field,  float* expandFactors)
{

  if (!field) return NULL;
  FieldExpanderPointer fieldxp;
  fieldxp = FieldExpanderType::New(); 
  fieldxp->SetInput(field);
  fieldxp->SetExpandFactors( expandFactors );
  fieldxp->UpdateLargestPossibleRegion();

  return fieldxp->GetOutput();
