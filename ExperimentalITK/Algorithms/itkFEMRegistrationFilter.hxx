  ...
  // Outputs the displacement field for the index provided (0=x,1=y,2=z)
{
  ...
  std::string outfile=m_DisplacementsFileName+static_cast<char>('x'+index)+std::string("vec."+m_FileExtension);
    ...
    if ( this->m_LandmarkArray.empty() ) 
      ...
      if (f)
        ...
        try
          ...
          { 
          std::cerr << "Exception: cannot read load landmark FEMRegistrationFilter.hxx " << err;
          }
        f.close();
...
void FEMRegistrationFilter<TMovingImage,TFixedImage>::WriteWarpedImage(const char* fname)
{
  ...
  // for image output
  std::ofstream fbin; 
  //std::string exte="."+m_FileExtension;
  std::string fnum;
  ...
  std::string fullfname=(fname+fnum+"."+m_FileExtension);
