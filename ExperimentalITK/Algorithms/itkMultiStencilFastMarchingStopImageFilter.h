/*=========================================================================
  ...
  Module:    $RCSfile: itkMultiStencilFastMarchingStopImageFilter.h,v $
  Language:  C++
  Date:      $Date: 2010/02/05 23:40:35 $
...
=========================================================================*/
#ifndef __itkMultiStencilFastMarchingStopImageFilter_h
#define __itkMultiStencilFastMarchingStopImageFilter_h
...
/** \class MultiStencilFastMarchingStopImageFilter
 ...
 * MultiStencilFastMarchingStopImageFilter is used in the ReinitializeLevelSetImageFilter
...
class ITK_EXPORT MultiStencilFastMarchingStopImageFilter :
    public ImageToImageFilter<TSpeedImage,TLevelSet>
{
public:
  /** Standard class typdedefs. */
  typedef MultiStencilFastMarchingStopImageFilter   Self;
  ...
  itkTypeMacro(MultiStencilFastMarchingStopImageFilter, ImageSource);
  ...
  class AxisNodeType : public NodeType
    ...
    public:
      ...
      signed char GetLeftOrRight() const { return m_Left_or_Right; }
      void SetLeftOrRight( signed char left_or_right ) {m_Left_or_Right = left_or_right; }
      const AxisNodeType & operator=(const NodeType & node)
      { this->NodeType::operator=(node); return *this; }
    private:
      int m_Axis;
      signed char  m_Left_or_Right; // 0: left, 1: right, same order in the definition of Stencil
  };
  ...
  void SetStoppingValue(double v){
      m_StoppingValue = v;
      // m_LargeValue = v + 100.0;
  }
  ...
  // added by songgang, temp variable for
     itkSetMacro( TrackTimeSource, bool );
     itkGetMacro( TrackTimeSource, bool );
     itkBooleanMacro(TrackTimeSource);
...
protected:
  MultiStencilFastMarchingStopImageFilter();
  ~MultiStencilFastMarchingStopImageFilter(){};
...
private:
  MultiStencilFastMarchingStopImageFilter(const Self&); //purposely not implemented
...
// added by songgang, for testing stop criterion
public:
  typedef LevelSetImageType IntensityImageType;
  typedef typename IntensityImageType::Pointer IntensityImagePointerType;
  IntensityImagePointerType     m_IntensityImage;
  typename IntensityImageType::PixelType    m_FreezeLowestIntensity;

  // for debug only, the index to watch in the fast marching functions
  int DBGINDEX[3];
// typedef FixedArray<signed short, 3> TimeSourceVectorType;
  typedef Vector<short, 3> TimeSourceVectorType;
  typedef Image<TimeSourceVectorType, SetDimension> TimeSourceImageType;
  typedef typename TimeSourceImageType::Pointer TimeSourceImagePointerType;
  TimeSourceImagePointerType  m_TimeSourceImage;
  bool m_TrackTimeSource;
  void AllocateTimeSourceImage();
